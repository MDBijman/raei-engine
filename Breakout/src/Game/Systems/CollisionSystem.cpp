...
void systems::collision_system::update(ecs_manager& ecs)
{
...
bool systems::collision_system::contains(const std::array<glm::vec2, 4>& cube, const glm::vec2& p)
{
	return cube[0].x < p.x && p.x < cube[2].x
		&& cube[0].y < p.y && p.y < cube[1].y;
}

bool systems::collision_system::intersects(const std::array<glm::vec2, 4>& c1, const std::array<glm::vec2, 4>& c2)
{
...
void systems::collision_system::check_collisions_for(ecs_manager& ecs, uint32_t entity, const std::unordered_set<uint32_t>& candidates)
{
